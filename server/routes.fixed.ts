...
`;
        ...
        if (shouldUseChunking) {
          ...
          switch (effectiveProvider) {
            ...
            default:
              ...
              result = await generatePerplexityResponse({
                model: 'llama-3.1-sonar-small-128k-online', // Default to large context model
                temperature,
                assignmentText,
                gradingText,
                studentText,
  ...
  // API endpoint to compare two submissions
  app.post('/api/compare-assignments', async (req: Request, res: Response) => {
    ...
    try {
      // Basic validation
      if (!provider || !model || !assignmentText || !submission1 || !submission2) {
        return res.status(400).json({ message: 'Missing required fields' });
      }
      
      // Extract submission text from the objects
      const submission1Text = typeof submission1 === 'object' && submission1 !== null 
        ? submission1.text || JSON.stringify(submission1)
        : String(submission1);
      
      const submission2Text = typeof submission2 === 'object' && submission2 !== null
        ? submission2.text || JSON.stringify(submission2)
        : String(submission2);
...
`;
  ...
  // API check for OpenAI, Anthropic and Perplexity
  app.get('/api/check-services', async (_req: Request, res: Response) => {
    const results = {
      openai: "unchecked",
      anthropic: "unchecked",
      perplexity: "unchecked"
    };
    ...
    try {
      ...
      if (openaiApiKey) {
        try {
          ...
          const testResult = await generateOpenAIResponse({
            model: "gpt-4o",
            temperature: 0.7,
            assignmentText: "Test prompt",
            gradingText: "Test instructions",
          ...
          if (testResult) {
            ...
            results.openai = "error: no response";
            console.error("OpenAI API test failed: No response");
          }
        } catch (error: any) {
      ...
      if (anthropicApiKey) {
        try {
          ...
          const testResult = await generateAnthropicResponse({
          ...
          if (testResult) {
            ...
            results.anthropic = "error: no response";
            console.error("Anthropic API test failed: No response");
          }
        } catch (error: any) {
        ...
        console.warn("No Anthropic API key found");
        results.anthropic = "no API key";
      }
      ...
      if (perplexityApiKey) {
        try {
          ...
          if (perplexityResult.ok) {
            ...
            const errorData = await perplexityResult.json();
            const errorMsg = errorData.error?.message || perplexityResult.statusText;
            console.error("Perplexity API error:", errorMsg);
            results.perplexity = `error: ${errorMsg}`;
          }
        } catch (error: any) {
          console.error("Perplexity test error:", error);
          results.perplexity = `error: ${error?.message || "Unknown error"}`;
        }
      } else {
        console.warn("No Perplexity API key found");
        results.perplexity = "no API key";
      }
    } catch (error: any) {
